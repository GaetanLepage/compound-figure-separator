...
RE_CHARACTERS: Pattern[str] = re.compile(
    r"((\b|\.|\()(" + _ANY_CHAR + r")(?![^).:0-9])[0-9]?(\)|\.|:|\b)?)"
)

RE_ROMAN: Pattern[str] = re.compile(
    r"((\b|\.|\()(" + _ANY_ROMAN + r")+(?![^).:])(\)|\.|:|\b)?)"
)

RE_DIGITS: Pattern[str] = re.compile(
    r"((\b|\.|\()(" + _ANY_NON_ZERO_DIGIT + r")(?![^).:0-9])[0-9]?(\)|\.|:|\b)?)"
)

# Build the regular expressions that match ranges (hyphens and conjunctions).
RE_HYPHEN: Pattern[str] = re.compile(
    r"((\b|\.|\()(" + _ANY_LABEL + r")[0-9]?-(" + _ANY_LABEL + r")[0-9]?\)?)\b"
)
...
# => single labels
RE_CHARACTERS_POS: Pattern[str] = re.compile(
    r"((in|from|panel(s)?)\s?\(?(" + _ANY_CHAR + r")(?![^).:0-9])[0-9]?(\)|\.|:|\b)?)"
)

RE_ROMAN_POS: Pattern[str] = re.compile(
    r"((in|from|panel(s)?)\s?\(?(" + _ANY_ROMAN + r")+(?![^).:])(\)|\.|:|\b)?)"
)

RE_DIGITS_POS: Pattern[str] = re.compile(
    r"((in|from|panel(s)?)\s?\(?(" + _ANY_NON_ZERO_DIGIT + r")(?![^).:0-9])[0-9]?(\)|\.|:|\b)?)"
)

# => range labels
RE_HYPHEN_POS: Pattern[str] = re.compile(
